...
# This array contains the neighborhoods we want to get crime info from
...
mapURL = "<img src = 'https://maps.googleapis.com/maps/api/staticmap?zoom=13&center=university+of+cincinnati&size=370x330&scale=2&maptype=roadmap&markers=color:blue%7Clabel:"
offCampusArray = []

for i in 0...neighborhoodArray.length
	sleep 4
	# Sleep so we aren't persistently bothering the website
	begin
		...
		if retries > 0
			...
			crimeHTML += '<p>The SpotCrime website is currently down.</p><p>Please be sure to check <a href="http://spotcrime.com">the SpotCrime web portal</a> later today or tomorrow for any updates.</p>'
			break
		end
	else
		...
		for j in 0...crime_blocks.length
			...
			for k in 0...crimes_in_block.length
				...
				if crime_date === yesterday
					crime_info = Hash.new 
					crime_info["detail_link"] = crimes_in_block[k]['href']
					crime_info["crime_type"] = crimes_in_block[k].css('h4').text
					crime_info["crime_date"] = crime_date
					crime_info["address"] = crimes_in_block[k].css('.crime-address').text
					offCampusArray << crime_info
				elsif crime_date < yesterday
					break
				end
			end
		end
...
if offCampusArray.length > 0
	...
	for m in 0...offCampusArray.size
		...
		crimeTableInfo += '<tr>'
		crimeTableInfo += '<td>' + crime['crime_date'].strftime("%m/%d/%Y") + '</td>'
		crimeTableInfo += '<td>' + crime['crime_type'] + '</td>'
		crimeTableInfo += '<td>' + crime['address'] + '</td>'
		crimeTableInfo += '<td>' + 'http://www.spotcrime.com' + crime['detail_link'] + '</td>'
		crimeTableInfo += '</tr>'
		...
		if location.include? " "
			mapURL += "%7C" + location.gsub!(/\s+/, '+') + "+Cincinnati+Ohio"
		else
			mapURL += "%7C" + location + "+Cincinnati+Ohio"
		end
	end
	# End table
